...
func registerProfilesTools(s *server.MCPServer, engine crypto.MaknoonEngine) {
	s.AddTool(mcp.NewTool("profiles_list",
		mcp.WithDescription("List all built-in and custom cryptographic profiles"),
	), func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		conf := engine.GetConfig()

		var res crypto.ProfileListResult
		res.Profiles = append(res.Profiles,
			crypto.ProfileInfo{Name: "nist", ID: 1, Description: "NIST PQC (ML-KEM + ML-DSA)"},
			crypto.ProfileInfo{Name: "conservative", ID: 3, Description: "Non-Lattice PQC (FrodoKEM + SLH-DSA)"},
		)
	...
	s.AddTool(mcp.NewTool("profiles_gen",
		mcp.WithDescription("Generate a new random, validated cryptographic profile and save it to config"),
		mcp.WithString("name", mcp.Required(), mcp.Description("Name for the new profile")),
	), func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		conf := engine.GetConfig()
		usedIDs := map[byte]bool{1: true, 2: true, 3: true}
	...
	s.AddTool(mcp.NewTool("profiles_rm",
		mcp.WithDescription("Remove a custom profile from config"),
		mcp.WithString("name", mcp.Required(), mcp.Description("Profile name to remove")),
	), func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		args := getArgs(request)
		name := getString(args, "name", "")
