...
**Man pages:** 72 per-command man pages in `man/` (generated by `cobra/doc.GenManTree`).
...
make man             # regenerate all 72 man pages into man/
...
maknoon fragment secret.makn --data 5 --parity 3 \
    ...
    --chunk-size 131072    # optional: override 64 KB default (4 KB–4 MB)
...
| Version | Size | Layout |
|---|---|---|
| V1 | 16 B | `MAKF(4) \| Ver(1) \| Index(1) \| Data(1) \| Parity(1) \| OrigSize(8)` |
| V2 | 18 B | V1 + `SigSize(2)` — self-describing signature size |
| V3 | 22 B | V2 + `ChunkSize(4)` — configurable shard chunk size |

V3 is written when `--chunk-size` differs from the 64 KB default. All versions reassemble correctly.
...
```json
{
  ...
  "signed": false, "shard_chunk_size": 131072,
  "shards": [{"index": 0, "filename": "shard_000.maknf"}, ...]
}
```
`shard_chunk_size` is omitted (0) when using the default 64 KB chunk size.  
